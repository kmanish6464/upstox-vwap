...
ARCHITECTURE
  ...
  • Bot guards (all apply to BUY_CALL and BUY_PUT only):
      ...
      3. S/R no-trade zone     : no entry when Nifty futures LTP is between
...
4. config.ini:
     ...
     Nifty_futures_key   = NSE_FO|NIFTY25APRFUT  <- current-month Nifty futures
...
NIFTY_KEY   = config.get("UPSTOX", "Nifty_futures_key",    fallback="NSE_FO|NIFTY25APRFUT")  # current-month Nifty futures
...
def get_nifty_futures_ltp() -> float | None:
    """Fetch current Nifty current-month futures LTP for the S/R zone check."""
...
def _in_sr_no_trade_zone() -> tuple[bool, float | None]:
    """
    Fetches Nifty current-month futures LTP and checks if it is inside
    the S/R no-trade zone.
    Returns (blocked: bool, futures_ltp: float | None).
...
def route_signal(raw_signal: str, price: float | None, sig_time: str) -> dict:
    ...
    with lock:
        ...
        if action in ("BUY_CALL", "BUY_PUT"):
            ...
            if blocked:
                reason = (f"BLOCKED {action} — Nifty futures LTP {spot:.2f} is inside "
