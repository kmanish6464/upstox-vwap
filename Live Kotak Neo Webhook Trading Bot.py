...
KOTAK_CONSUMER_SECRET = config.get('KOTAK', 'consumer_secret', fallback='')
KOTAK_MOBILE = config.get('KOTAK', 'mobile_number', fallback='')
KOTAK_PASSWORD = config.get('KOTAK', 'password', fallback='')
KOTAK_MPIN = config.get('KOTAK', 'mpin', fallback='')
...
try:
    ...
    kotak_client.login(mobilenumber=KOTAK_MOBILE, password=KOTAK_PASSWORD)
    kotak_client.session_2fa(OTP=KOTAK_MPIN)
    logger.info("Kotak Neo Client authenticated and initialized successfully for LIVE TRADE.")
except Exception as e:
    logger.error(f"Failed to initialize Kotak Neo Client: {e}")
    logger.error("Check your credentials in config.ini (Mobile, Password, MPIN, Keys).")
...
def place_order(symbol, side):
    ...
    try:
        ...
        if isinstance(res, dict) and 'nOrdNo' in res:
            logger.info(f"LIVE Order Placed: {side} {symbol} | Order No: {res['nOrdNo']}")
        else:
...
def health_check():
    ...
    return jsonify({"status": "healthy", "timestamp": str(now_ist())})
...
def reset_stats():
    ...
    return jsonify({"status": "success", "message": "Daily stats reset successfully."})
...
if __name__ == '__main__':
    banner = (
        ...
        "==========================================================\n"
        f"|  Time IST      : {now_ist()}\n"
        f"|  Live Trading  : ENABLED (Prod Environment)\n"
        "+----------------------------------------------------------+\n"
