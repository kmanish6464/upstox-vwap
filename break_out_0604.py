...
================================================================
  ...
  Mode    : Continuous Market Hours (09:15 - 15:35)
================================================================
"""
...
SESSION_END = "15:30"
DELAY_SEC = 0.3
POLL_INTERVAL_MIN = 5  # Minutes to sleep between full scan cycles
...
def save_csv(rows: list[dict], mode="a"):
    if not rows: return
...
def main():
    print("=" * 65)
    print("  STRICT VWAP + EMA BREAKOUT SCANNER (CONTINUOUS MODE)")
    ...
    # This dictionary ensures we only record ONE valid setup per stock per day
    triggered_stocks = {}

    print(f"\n  [ READY ] Scanning {total} symbols.")
    print("  Press Ctrl+C at any time to safely stop and print the summary.\n")

    try:
        while True:
            now = datetime.now()
            scan_date = now.strftime("%Y-%m-%d") if TARGET_DATE.lower() == "today" else TARGET_DATE
            from_date = (datetime.strptime(scan_date, "%Y-%m-%d") - timedelta(days=LOOKBACK_DAYS)).strftime("%Y-%m-%d")

            is_live = (TARGET_DATE.lower() == "today")
            market_start = now.replace(hour=9, minute=15, second=0, microsecond=0)
            market_end = now.replace(hour=15, minute=35, second=0, microsecond=0)

            # 1. Wait for Market Open
            if is_live and now < market_start:
                wait_sec = (market_start - now).total_seconds()
                sys.stdout.write(f"\r  Market opens at 09:15. Waiting {int(wait_sec)}s... ")
                sys.stdout.flush()
                time.sleep(min(wait_sec, 60))
                continue

            # 2. Stop if Market Closed
            if is_live and now > market_end:
                print("\n  Market is closed. Generating final summary...")
                break

            print(f"\n  [ {now.strftime('%H:%M:%S')} ] Starting new scan cycle...")
            tg(f"<b>Scan Cycle Started</b>\nTime: {now.strftime('%H:%M:%S')}")

            # 3. Scanning Loop
            for i, (sym, ikey) in enumerate(symbols, 1):
                sys.stdout.write(f"\r  [{i:>4}/{total}]  {sym:<14}  Unique Signals Today: {len(triggered_stocks)}")
                sys.stdout.flush()
                ...
                for s in sigs:
                    # ONLY ONE ENTRY PER STOCK rule enforced here
                    if sym not in triggered_stocks:
                        triggered_stocks[sym] = s
                        print()
                        print_signal(s)
                        send_signal(s)
                        save_csv([s], mode="a")  # Appends to CSV instantly so you don't lose data

            # If scanning a historical specific date, we don't need to loop continuously
            if not is_live:
                break

            # 4. Rest before next cycle
            print(f"\n  Cycle finished. Sleeping for {POLL_INTERVAL_MIN} minutes...")
    ...
    # ================================================================
    # END OF DAY FINAL SUMMARY (Only Prints ONCE)
    # ================================================================
    print(f"\n{'─' * 65}")
    if triggered_stocks:
        print(f"\n  FINAL SUMMARY : {len(triggered_stocks)} UNIQUE signal(s) found today\n")
        print(f"  {'Symbol':<13} {'Time':<6} {'Close':>8} {'EMA100':>8} {'EMA200':>8} {'Setup'}")
        print("  " + "─" * 70)

        for sym, s in triggered_stocks.items():
            print(f"  {s['Symbol']:<13} {s['Time']:<6} {s['Close']:>8.2f} "
                  f"{s['EMA100']:>8.2f} {s['EMA200']:>8.2f}  {s['Setup_Type']}")

        lines = [f"<b>Market Closed — Final Summary</b>", f"{len(triggered_stocks)} UNIQUE STRICT signal(s)\n"]
        for sym, s in triggered_stocks.items():
            lines.append(f"<code>{s['Symbol']:<12}</code> {s['Time']}  ₹{s['Close']} ({s['Setup_Type']})")
        tg("\n".join(lines))
    else:
        print(f"\n  No valid setups found today.")
        tg(f"<b>Market Closed</b>\nNo signals passed strict filters today.")
